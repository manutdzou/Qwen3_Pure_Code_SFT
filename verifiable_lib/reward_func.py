...
def validate_repeat_prompt(text: str, original_prompt: str) -> bool:
    #zoujy2 modify
    if text.startswith(original_prompt.split("First, repeat the request without change, then give your answer (do not say anything before repeating the request; the request you need to repeat does not include this sentence)")[-1].strip()):
        return True
    else:
        return False
...
class IFEvalVerifier(VerifierFunction):
    ...
    def __call__(self, tokenized_prediction: List[int], prediction: str, label: Union[str, Dict]) -> bool:
        constraint = label
        answer = prediction.split("<|im_end|>")[0].split("</think>\n\n")[-1].strip()
        ...
        if not constraint:
            return bool(func(prediction))
        return bool(func(answer, **non_none_args))
...
if __name__ == "__main__":
    ...
    reward_function = IFEvalVerifier()

    datas = get_json_list2("RLVR-iFeval.json")
    ...
    save_path = os.path.join(".", "t.json")
    jsonl_string = "\n".join(json.dumps(item, ensure_ascii=False) for item in tdata)
    with open(save_path, 'w', encoding='utf-8') as file:
        file.write(jsonl_string)
    save_path = os.path.join(".", "f.json")
    jsonl_string = "\n".join(json.dumps(item, ensure_ascii=False) for item in fdata)
    with open(save_path, 'w', encoding='utf-8') as file:
        file.write(jsonl_string)
